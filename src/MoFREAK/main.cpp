...
double classify()
{
	...
	std::ofstream output_file(results_file);
	...
	for (unsigned i = 0; i < training_files.size(); ++i)
	{
		...
		std::ifstream response_file(svm_out);
		string line;
		...
		std::ifstream truth_file(test_filename);
...
void detectEvents()
{
	...
	for (auto it = response_files.begin(); it != response_files.end(); ++it)
	{
		...
		// print to file
		std::ofstream detection_stream(*it + ".detections");
