...
void BagOfWordsRepresentation::loadClusters()
{
	...
	std::string cluster_path = SVM_PATH + "/clusters.txt";

	ifstream cluster_file(cluster_path);
	if ((!cluster_file.is_open()) || cluster_file.bad())
	{
		std::cout << "could not open clusters file" << std::endl;
		exit(1);
	}
...
void BagOfWordsRepresentation::computeSlidingBagOfWords(std::string &file, int alpha, int label, std::ofstream &out)
{

	bool over_alpha_frames = false;
	std::string distance_file = file;
...
int BagOfWordsRepresentation::actionStringToActionInt(std::string act)
{
	if (act == "brush_hair")
	{
		return BRUSH_HAIR;
	...
	{
		std::cout << "****Didn't find action: " << act << std::endl;
		system("PAUSE");
		exit(1);
		return BRUSH_HAIR;
	}
...
void BagOfWordsRepresentation::computeHMDB51BagOfWords(std::string SVM_PATH, std::string MOFREAK_PATH, std::string METADATA_PATH)
{
	/*
	...
	*/
	...
	std::vector<std::string> train1_lines;
	std::vector<std::string> train2_lines;
	std::vector<std::string> train3_lines;
	std::vector<std::string> test1_lines;
	std::vector<std::string> test2_lines;
	std::vector<std::string> test3_lines;
	...
	*/
	...
	for (directory_iterator dir_iter(METADATA_PATH); dir_iter != end_iter; ++dir_iter)
	{
		if (is_regular_file(dir_iter->status()))
		{
			...
			std::string action_filename = current_file.filename().string();
			int pos = action_filename.find("_test_");
			std::string str_action = action_filename.substr(0, pos);
			std::cout << "Action: " << str_action << std::endl;
			int action = actionStringToActionInt(str_action);

			int split;
			pos = action_filename.find(".txt");
			std::istringstream(action_filename.substr(pos - 1, 1)) >> split;
			...
			*/
			...
			std::cout << "current file: " << current_file.string() << "..." << std::endl;
			...
			while (std::getline(metadata_file, line))
			{
				...
				std::string mofreak_file = line.substr(0, last_space_index - 2) + ".mofreak";
				std::string label = line.substr(line.length() - 2, 1);
...
#pragma omp parallel for
			for (int j = 0; j < training_files.size(); ++j)
			{
				...
				case 3:
					...
					break;
				default:
					std::cout << "Couldn't get right split! " << split << std::endl;
					break;
				}
...
void BagOfWordsRepresentation::intializeBOWMemory(std::string SVM_PATH)
{

	// initialize the output files and memory for BOW features for each grouping.
	for (int group = 0; group < NUMBER_OF_GROUPS; ++group)
	{
		std::stringstream training_filepath, testing_filepath;
		...
		std::vector<std::string> training_features;
		std::vector<std::string> testing_features;
...
void BagOfWordsRepresentation::writeBOWFeaturesToFiles()
{
	...
	{
		std::cout << "Incorrect number of training or testing file lines.  Check mapping from bow feature to test/train files" << std::endl;
		exit(1);
	}
	...
	{
		std::cout << "number of training features: " << bow_training_crossvalidation_sets[i].size() << std::endl;
		for (unsigned line = 0; line < bow_training_crossvalidation_sets[i].size(); ++line)
		{
			try
			{
				*training_files[i] << bow_training_crossvalidation_sets[i][line] << std::endl;
			}
		...
		std::cout << "number of testing features: " << bow_testing_crossvalidation_sets[i].size() << std::endl;
		for (unsigned line = 0; line < bow_testing_crossvalidation_sets[i].size(); ++line)
		{
			try
			{
				*testing_files[i] << bow_testing_crossvalidation_sets[i][line] << std::endl;
			}
			catch (std::exception &e)
			{
				std::cout << "Error: " << e.what() << std::endl;
	...
	std::cout << "Finished writing to cross-validation files." << std::endl;
	...
	std::cout << "Closed all cross-validation files. " << std::endl;

	training_files.clear();
	testing_files.clear();
}

void BagOfWordsRepresentation::computeBagOfWords(std::string SVM_PATH, std::string MOFREAK_PATH, std::string METADATA_PATH)
{
	if (dataset == HMDB51)
	{
	...
	if (dataset == KTH || dataset == UCF101)
	{
		for (int i = 0; i < NUMBER_OF_GROUPS; ++i)
		{
			std::stringstream training_string;
			std::stringstream testing_string;
	...
	std::cout << "parallelizing bag-of-words matches across files." << std::endl;
#pragma omp parallel for
	for (int i = 0; i < files.size(); ++i)
	{
		convertFileToBOWFeature(files[i]);
	}
	std::cout << "done bag-of-words feature construction" << std::endl;
